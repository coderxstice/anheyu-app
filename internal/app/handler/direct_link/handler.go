...
type DirectLinkHandler struct {
	svc              *direct_link.DirectLinkService
	storageProviders map[constant.StoragePolicyType]storage.IStorageProvider
}

// NewDirectLinkHandler 是 DirectLinkHandler 的构造函数。
func NewDirectLinkHandler(svc *direct_link.DirectLinkService, providers map[constant.StoragePolicyType]storage.IStorageProvider) *DirectLinkHandler {
